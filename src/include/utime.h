...
/* WARNING: If add member in utime_mono_t, please make sure the encode/decode funtion
 * work well. For little-endian machine, we should make sure there is no padding
 * in 32-bit machine and 64-bit machine.
 * You should also modify the padding_check function.
 */
...
public:
 ...
 public:
  ...
  // cons
  utime_mono_t() { tv.tv_sec = 0; tv.tv_nsec = 0; }
  utime_mono_t(time_t s, int n) { tv.tv_sec = s; tv.tv_nsec = n; normalize(); }
  utime_mono_t(const struct ceph_timespec &v) {
    decode_timeval(&v);
  }
  utime_mono_t(const struct timespec v)
  {
    tv.tv_sec = v.tv_sec;
    tv.tv_nsec = v.tv_nsec;
  }
  utime_mono_t(const struct timeval &v) {
    set_from_timeval(&v);
  }
  utime_mono_t(const struct timeval *v) {
    set_from_timeval(v);
  ...
  utime_mono_t round_to_minute() {
    struct tm bdt;
    time_t tt = sec();
    gmtime_r(&tt, &bdt);
    bdt.tm_sec = 0;
    tt = mktime(&bdt);
    return utime_mono_t(tt, 0);
  }

  utime_mono_t round_to_hour() {
    struct tm bdt;
    time_t tt = sec();
    gmtime_r(&tt, &bdt);
    bdt.tm_sec = 0;
...
WRITE_CLASS_ENCODER(utime_mono_t)


// arithmetic operators
inline utime_mono_t operator+(const utime_mono_t& l, const utime_mono_t& r) {
  return utime_mono_t( l.sec() + r.sec() + (l.nsec()+r.nsec())/1000000000L,
                  (l.nsec()+r.nsec())%1000000000L );
}
inline utime_mono_t& operator+=(utime_mono_t& l, const utime_mono_t& r) {
  l.sec_ref() += r.sec() + (l.nsec()+r.nsec())/1000000000L;
  l.nsec_ref() += r.nsec();
  l.nsec_ref() %= 1000000000L;
  return l;
}
inline utime_mono_t& operator+=(utime_mono_t& l, double f) {
  double fs = trunc(f);
  double ns = (f - fs) * (double)1000000000.0;
  l.sec_ref() += (long)fs;
  l.nsec_ref() += (long)ns;
...
inline utime_mono_t operator-(const utime_mono_t& l, const utime_mono_t& r) {
  return utime_mono_t( l.sec() - r.sec() - (l.nsec()<r.nsec() ? 1:0),
                  l.nsec() - r.nsec() + (l.nsec()<r.nsec() ? 1000000000:0) );
}
inline utime_mono_t& operator-=(utime_mono_t& l, const utime_mono_t& r) {
  l.sec_ref() -= r.sec();
...
// comparators
inline bool operator>(const utime_mono_t& a, const utime_mono_t& b)
{
  return (a.sec() > b.sec()) || (a.sec() == b.sec() && a.nsec() > b.nsec());
}
inline bool operator<=(const utime_mono_t& a, const utime_mono_t& b)
...
// ostream
inline std::ostream& operator<<(std::ostream& out, const utime_mono_t& t)
{
  return t.localtime(out);
}
