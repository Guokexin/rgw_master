...
#ifndef CEPH_XJOURNAL_H
#define CEPH_XJOURNAL_H
...
private:
  ...
  class WriteFinisher : public Thread {
    XJournal *journal;
  public:
    WriteFinisher(XJournal *fj) : journal(fj) {}
    void *entry() {
 ...
 public:
  XJournal(uuid_d fsid, Finisher *fin, Cond *sync_cond, const char *f, bool dio=false, bool ai=true, bool faio=false) :
    Journal(fsid, fin, sync_cond),
    finisher_lock("XJournal::finisher_lock", false, true, false, g_ceph_context),
    journaled_seq(0),
    plug_journal_completions(false),
    writeq_lock("XJournal::writeq_lock", false, true, false, g_ceph_context),
    completions_lock(
      "XJournal::completions_lock", false, true, false, g_ceph_context),
    fn(f),
    zero_buf(NULL),
    ...
    write_lock("XJournal::write_lock", false, true, false, g_ceph_context),
    write_stop(false),
    aio_stop(false),
    write_thread(this),
...
WRITE_CLASS_ENCODER(XJournal::header_t)
