...
int XJournal::_open(bool forwrite, bool create)
{
  ...
  if (forwrite) {
    ...
    if (g_conf->journal_force_odsync) {
      ...
      derr << TEXT_RED
           << " ** WARNING: XJournal write without O_DSYNC. You should make" << ".\n"
  ...
  if (fd >= 0) {
    if (TEMP_FAILURE_RETRY(::close(fd))) {
      int err = errno;
      derr << "XJournal::_open: error closing old fd: "
  ...
  if (fd < 0) {
    int err = errno;
    dout(2) << "XJournal::_open unable to open journal "
  ...
  if (ret) {
    ret = errno;
    derr << "XJournal::_open: unable to fstat journal: " << cpp_strerror(ret) << dendl;
    ret = -ret;
    goto out_fd;
  }
  ...
  if (S_ISBLK(st.st_mode)) {
    ...
    if (aio && !force_aio) {
      derr << "XJournal::_open: disabling aio for non-block journal.  Use "
  ...
  if (aio) {
    ...
    if (ret < 0) {
      ret = errno;
      derr << "XJournal::_open: unable to setup io_context " << cpp_strerror(ret) << dendl;
      ret = -ret;
      goto out_fd;
    }
  }
...
int XJournal::_open_block_device()
{
  int64_t bdev_sz = 0;
...
void XJournal::_check_disk_write_cache() const
{
...
int XJournal::_open_file(int64_t oldsize, blksize_t blksize,
			    bool create)
{
  int ret;
  ...
  if (create && (oldsize < conf_journal_sz)) {
    ...
    if (ret < 0) {
      int err = errno;
      derr << "XJournal::_open_file : unable to extend journal to "
    ...
    if (ret) {
      derr << "XJournal::_open_file : unable to preallocation journal to "
...
# error "Journal pre-allocation not supported on platform."
  ...
  if (create && g_conf->journal_zero_on_create) {
    derr << "XJournal::_open_file : zeroing journal" << dendl;
...
int XJournal::create()
{
  ...
  if (TEMP_FAILURE_RETRY(::pwrite(fd, bp.c_str(), bp.length(), 0)) < 0) {
    ret = errno;
    derr << "XJournal::create : create write header error "
  ...
  if (ret) {
    derr << "XJournal::create: failed to allocate " << block_size
  ...
  if (TEMP_FAILURE_RETRY(::pwrite(fd, buf, block_size, get_top())) < 0) {
    ret = errno;
    derr << "XJournal::create: error zeroing first " << block_size
  ...
  if (header.max_size - header.start < needed_space) {
    derr << "XJournal::create: OSD journal is not large enough to hold "
...
close_fd:
  if (TEMP_FAILURE_RETRY(::close(fd)) < 0) {
    ret = errno;
    derr << "XJournal::create: error closing fd: " << cpp_strerror(ret)
	 << dendl;
    goto done;
  }
done:
...
int XJournal::peek_fsid(uuid_d& fsid)
{
...
int XJournal::open(uint64_t fs_op_seq)
{
  ...
  dout(10) << "open header.fsid = " << header.fsid 
    //<< " vs expected fsid = " << fsid 
	   << dendl;
  if (header.fsid != fsid) {
    derr << "XJournal::open: ondisk fsid " << header.fsid << " doesn't match expected " << fsid
         << ", invalid (someone else's?) journal" << dendl;
    return -EINVAL;
  }
...
bufferptr XJournal::prepare_header()
{
  bufferlist bl;
  {
...
int XJournal::prepare_multi_write(bufferlist& bl, uint64_t& orig_ops, uint64_t& orig_bytes)
{
...
/*
void XJournal::queue_write_fin(uint64_t seq, Context *fin)
{
...
void XJournal::queue_completions_thru(uint64_t seq)
{
...
int XJournal::prepare_single_write(write_item &next_write, bufferlist& bl, off64_t& queue_pos, uint64_t& orig_ops, uint64_t& orig_bytes)
{
...
void XJournal::align_bl(off64_t pos, bufferlist& bl)
{
...
int XJournal::write_bl(off64_t& pos, bufferlist& bl)
{
  int ret;
  ...
  if (spos < 0) {
    ret = -errno;
    derr << "XJournal::write_bl : lseek64 failed " << cpp_strerror(ret) << dendl;
    return ret;
  }
  ret = bl.write_fd(fd);
  if (ret) {
    derr << "XJournal::write_bl : write_fd failed: " << cpp_strerror(ret) << dendl;
    return ret;
  }
  pos += bl.length();
...
void XJournal::do_write(bufferlist& bl)
{
  // nothing to do?
  ...
  if (pos + bl.length() > header.max_size) {
    ...
    if (write_bl(pos, second)) {
      derr << "XJournal::do_write: write_bl(pos=" << orig_pos
	   << ") failed" << dendl;
      ceph_abort();
    }
    orig_pos = first_pos;
	...
	int err = errno;
	derr << "XJournal::do_write: pwrite(fd=" << fd
    ...
    if (write_bl(pos, bl)) {
      derr << "XJournal::do_write: write_bl(pos=" << pos
	   << ") failed" << dendl;
      ceph_abort();
    }
  }
...
void XJournal::write_thread_entry()
{
...
#ifdef HAVE_LIBAIO
void XJournal::do_aio_write(bufferlist& bl)
{
  ...
  if (pos + bl.length() > header.max_size) {
    ...
    if (write_aio_bl(pos, first, 0)) {
      derr << "XJournal::do_aio_write: write_aio_bl(pos=" << pos
	   << ") failed" << dendl;
      ceph_abort();
    }
	...
	derr << "XJournal::do_aio_write: write_aio_bl(header) failed" << dendl;
	ceph_abort();
      }
    }
 ...
 * on completion.
 */
int XJournal::write_aio_bl(off64_t& pos, bufferlist& bl, uint64_t seq)
{
  align_bl(pos, bl);
...
void XJournal::write_finish_thread_entry()
{
#ifdef HAVE_LIBAIO
 ...
 */
void XJournal::check_aio_completion()
{
...
XJournal::write_item &XJournal::peek_write()
{
...
void XJournal::batch_peek_write(list<write_item> &items)
{
  assert(write_lock.is_locked());
  Mutex::Locker locker(writeq_lock);
  writeq.swap(items);
...
void XJournal::backfill_write(list<write_item> &items)
{
  assert(write_lock.is_locked());
  Mutex::Locker locker(writeq_lock);
...
void XJournal::commit_start(uint64_t seq)
{
 ...
 */
void XJournal::do_discard(int64_t offset, int64_t end)
{
...
void XJournal::committed_thru(uint64_t seq)
{
  Mutex::Locker locker(write_lock);
...
void XJournal::put_throttle(uint64_t ops, uint64_t bytes)
{
  ...
  )
{
  while (olen > 0) {
    ...
    if (r) {
      derr << "XJournal::wrap_read_bl: safe_read_exact " << pos << "~" << len << " returned "
	   << r << dendl;
      ceph_abort();
    }
    bl->push_back(bp);
...
XJournal::read_entry_result XJournal::do_read_entry(
  off64_t init_pos,
  off64_t *next_pos,
  bufferlist *bl,
  uint64_t *seq,
...
void XJournal::corrupt_footer_magic(
  int wfd,
  uint64_t seq)
{
  off64_t pos = 0;
...
void XJournal::corrupt_header_magic(
  int wfd,
  uint64_t seq)
{
  off64_t pos = 0;
