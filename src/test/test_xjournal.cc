...
TEST(TestXJournal, Create) {
  fsid.generate_random();
  XJournal j(fsid, finisher, &sync_cond, path, directio, aio);
  ASSERT_EQ(0, j.create());
}

TEST(TestXJournal, WriteSmall) {
  fsid.generate_random();
  XJournal j(fsid, finisher, &sync_cond, path, directio, aio);
  ASSERT_EQ(0, j.create());
  j.make_writeable();
...
TEST(TestXJournal, WriteManyVecs) {
  fsid.generate_random();
  XJournal j(fsid, finisher, &sync_cond, path, directio, aio);
  ASSERT_EQ(0, j.create());
  j.make_writeable();
...
TEST(TestXJournal, ReplayCorrupt) {
  fsid.generate_random();
  XJournal j(fsid, finisher, &sync_cond, path, directio, aio);
  ASSERT_EQ(0, j.create());
  j.make_writeable();
...
TEST(TestXJournal, WriteTrimSmall) {
  fsid.generate_random();
  XJournal j(fsid, finisher, &sync_cond, path, directio);
  ASSERT_EQ(0, j.create());
  j.make_writeable();
...
TEST(TestXJournal, ReplayDetectCorruptFooterMagic) {
  ...
  bl.clear();
  bool corrupt = false;
  bool result = j.read_entry(bl, seq, &corrupt, (bool*)0);
  ASSERT_TRUE(result);
  ASSERT_EQ(seq, 1UL);
  ASSERT_FALSE(corrupt);

  result = j.read_entry(bl, seq, &corrupt, (bool*)0);
  ASSERT_FALSE(result);
  ASSERT_TRUE(corrupt);
...
TEST(TestXJournal, ReplayDetectCorruptPayload) {
  g_ceph_context->_conf->set_val("journal_ignore_corruption", "true");
  g_ceph_context->_conf->set_val("journal_write_header_frequency", "1");
  g_ceph_context->_conf->apply_changes(NULL);
...
TEST(TestXJournal, ReplayDetectCorruptHeader) {
  g_ceph_context->_conf->set_val("journal_ignore_corruption", "true");
  g_ceph_context->_conf->set_val("journal_write_header_frequency", "1");
  g_ceph_context->_conf->apply_changes(NULL);
